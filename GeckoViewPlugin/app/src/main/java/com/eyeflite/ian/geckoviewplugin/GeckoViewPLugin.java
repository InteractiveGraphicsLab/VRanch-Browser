...
public class GeckoViewPLugin extends Fragment implements GeckoSession.NavigationDelegate, GeckoSession.ContentDelegate, GeckoSession.HistoryDelegate, GeckoSession.TextInputDelegate, GeckoSession.ProgressDelegate, GeckoRuntime.Delegate {
    ...
    private ArrayList<GeckoSession> mSessions = new ArrayList<GeckoSession>();
...
//        Log.d("AndroidUnity","Add tap called! at x:" + x + " y: "+y);
    ...
    public void OpenNewSession(int id, String url, boolean active){
        final Activity a = UnityPlayer.currentActivity;
        a.runOnUiThread(() -> {
            Log.d("AndroidUnity", "OpenNewSession, id=" + id + ", url=" + url);
            ...
            GeckoSession newSession = InitNewSession(mRuntime);
    ...
    public void ChangeSession(int id){
        ...
        a.runOnUiThread(() -> {
            ...
            GeckoSession newSession = mSessions.get(id);
    ...
    */
     ...
     * @param metaState
     */
    public void AddKeyCode(int code, int metaState, int repeat){
        final Activity a = UnityPlayer.currentActivity;
        a.runOnUiThread(() -> {
...
//            }
    ...
    public void ExitWebview(){
        final Activity a = UnityPlayer.currentActivity;
        a.runOnUiThread(new Runnable() {public void run() {
            Log.d("AndroidUnity","ExitWebview");
            mWebView.releaseSession();
...
//      Instance.mWebView.UnityCallback = Instance.UnityCallback;
  ...
  public void PassSurface(final Surface surface ){
    ...
    public void SetBrowserUserAgent(String agent, boolean reload){
        ...
        GeckoSession session = mSessions.get(0);
    ...
    private GeckoSession InitNewSession(GeckoRuntime runtime){
        Log.d("AndroidUnity", "InitNewSession");
...
//        Log.d(LOG_TAG, "location change uri: " + url);
    ...
    public GeckoResult<AllowOrDeny> onLoadRequest(@NonNull GeckoSession session, @NonNull LoadRequest request) {
        ...
        final Activity a = UnityPlayer.currentActivity;
        a.runOnUiThread(new Runnable() {public void run() {
            UnityCallback.OnLoadRequest(request.uri);
        }});
        Log.d("AndroidUnity", "hasUserGesture: "+request.hasUserGesture+", isDirectNavigation: "+request.isDirectNavigation+", target: "+request.target+"triggerUri: "+request.triggerUri);
